...
func printGuestInfo(guest: Guest) {
    print(guest.name + " " + String(guest.age))
}
...
func printAllGuestInfo(guests: [Guest]) {
    for guest in guests {
        print(guest.name + " " + String(guest.age))
    }
}
...
printAllGuestInfo(guests: invitedGuests)
...
func checkGuestAges(guests: [Guest]) -> [Guest] {
    var adultGuests: [Guest] = []
    
    for guest in guests {
        if guest.age >= 18 {
            adultGuests.append(guest)
        }
    }
...
print(checkGuestAges(guests: invitedGuests))
/*:
